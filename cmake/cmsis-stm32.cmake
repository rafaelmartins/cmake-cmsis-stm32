...
function(cmsis_stm32_target target)
    ...
    if(target_arg_STLINK)
        ...
        if(NOT ST_FLASH)
            ...
            if(DEFINED ${target}_STLINK_RESET)
                set(${target}_STLINK_RESET "${${target}_STLINK_RESET}" CACHE BOOL "${target} stlink tools reset")
            else()
                set(${target}_STLINK_RESET ON CACHE BOOL "${target} stlink tools reset")
            endif()
            if(${target}_STLINK_RESET)
                set(STLINK_RESET_ARG "--reset")
            endif()

            set(${target}_STLINK_CONNECT_UNDER_RESET "${${target}_STLINK_CONNECT_UNDER_RESET}" CACHE BOOL "${target} stlink tools connect under reset")
            if(${target}_STLINK_CONNECT_UNDER_RESET)
                set(STLINK_CONNECT_UNDER_RESET_ARG "--connect-under-reset")
            endif()

            set(${target}_STLINK_HOTPLUG "${${target}_STLINK_HOTPLUG}" CACHE BOOL "${target} stlink tools hot plug")
            if(${target}_STLINK_HOTPLUG)
                set(STLINK_HOTPLUG_ARG "--hot-plug")
            endif()

            set(${target}_STLINK_FREQ "${${target}_STLINK_FREQ}" CACHE STRING "${target} stlink tools frequency in khz")
            if(NOT ${target}_STLINK_FREQ STREQUAL "")
                set(STLINK_FREQ_ARG "--freq=${${target}_STLINK_FREQ}")
            endif()

            set(${target}_STLINK_SERIAL "${${target}_STLINK_SERIAL}" CACHE STRING "${target} stlink tools serial number (from st-info --serial)")
            if(NOT ${target}_STLINK_SERIAL STREQUAL "")
                set(STLINK_SERIAL_ARG "--serial=${${target}_STLINK_SERIAL}")
            endif()
